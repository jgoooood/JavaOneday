		...
		while(true) {
			...
			case 1: 
				...
				if(chosenRoom > 0 && chosenRoom <= 10) {
					if(!rooms[chosenRoom-1]) { // rooms[chosenRoom-1] false면 입실가능
			...
			case 2: 
				...
				chosenRoom = sc.nextInt();
				if(chosenRoom > 0 && chosenRoom <= 10) {
					if(rooms[chosenRoom-1]) { //rooms[chosenRoom-1]가 true면 퇴실가능
						rooms[chosenRoom-1] = false;
						System.out.println(chosenRoom+"번방에서 퇴실하셨습니다.");						
					} else {
						System.out.println(chosenRoom +"번 방은 현재 빈방입니다.");
					}
					Thread.sleep(250);
				} else {
					System.out.println("1번 방부터 10번방까지 준비되어 있습니다.");
